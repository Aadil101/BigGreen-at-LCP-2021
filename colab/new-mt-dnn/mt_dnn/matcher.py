...
class SANBertNetwork(nn.Module):
    ...
    def encode(self, input_ids, token_type_ids, attention_mask, head_mask=None):
        if head_mask is None:
            ...
            all_hidden_states = outputs[2]
            attention = outputs[-1]
            return sequence_output, pooled_output, all_hidden_states, attention
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, premise_mask=None, hyp_mask=None, task_id=0, fwd_type=0, embed=None, head_mask=None):
        if fwd_type == 2:
            ...
            sequence_output, pooled_output, _, attention = self.encode(input_ids, token_type_ids, attention_mask, head_mask=head_mask)
        decoder_opt = self.decoder_opt[task_id]
        task_type = self.task_types[task_id]
